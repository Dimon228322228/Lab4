...
public class JwtUtils {
    ...
    public boolean validateToken(String token){
        try {
            Jwts.parserBuilder().setSigningKey(Keys.hmacShaKeyFor(jwtSecret.getBytes())).build().parseClaimsJwt(token);
            return true;
        } catch (Exception e){
            return false;
        }
    ...
    public String getUserName(String token){
        return Jwts.parserBuilder().setSigningKey(Keys.hmacShaKeyFor(jwtSecret.getBytes())).build().parseClaimsJwt(token).getBody().getSubject();
    }
}
